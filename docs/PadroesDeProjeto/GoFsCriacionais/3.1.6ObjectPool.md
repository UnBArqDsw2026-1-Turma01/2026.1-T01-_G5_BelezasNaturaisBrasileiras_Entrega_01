...
![Diagrama - object_pool](../../assets/fase_3/object_poll.png)
...
### `ChatObjectPoolService` — pool com limite de 50 conexões

> [`backend/src/modules/chat/pool/chat-object-pool.service.ts`](https://github.com/UnBArqDsw2026-1-Turma01/2026.1-T01-_G5_BelezasNaturaisBrasileiras_Entrega_01/blob/branch-adapter-object_pool-mediator/backend/src/modules/chat/pool/chat-object-pool.service.ts)

```typescript
@Injectable()
export class ChatObjectPoolService implements IObjectPool<IChatConnection> {
...
### `ChatSessionManagerService` — acquire/release automático via `try/finally`

> [`backend/src/modules/chat/chat-session.manager.service.ts`](https://github.com/UnBArqDsw2026-1-Turma01/2026.1-T01-_G5_BelezasNaturaisBrasileiras_Entrega_01/blob/branch-adapter-object_pool-mediator/backend/src/modules/chat/chat-session.manager.service.ts)
...
export class ChatSessionManagerService {
  ...
  async withConnection<T>(
    callback: (conn: IChatConnection) => Promise<T>,
  ): Promise<T> {
...
| Integrante                                          | Revisão                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| :-------------------------------------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [Vitor Hoffmann](https://github.com/vitor-hoffmann) | O pool usa `pop()`/`push()`, operando como pilha LIFO — em fluxo sequencial, as mesmas conexões são reutilizadas com mais frequência, concentrando desgaste. Uma fila FIFO distribuiria o uso de forma mais uniforme. O `acquire()` descarta silenciosamente conexões mortas e cria novas sem contabilizar reconexões; um contador auxiliaria no monitoramento de instabilidade. O `cleanIdle()` fecha todas as conexões, mas os trechos de código não mostram se há um hook `onModuleDestroy` registrado — sem ele, as conexões não são encerradas no shutdown da aplicação. |
| [Ana Luiza Pfeilsticker](https://github.com/ana-pfeilsticker) | O padrão `withConnection(callback)` com `try/finally` é a decisão de design mais importante desta implementação — garante que nenhuma conexão fique abandonada mesmo em caso de exceção no callback, prevenindo leaks. O comportamento LIFO (`push`/`pop`) concentra reutilização nas conexões mais recentes enquanto as mais antigas ficam ociosas indefinidamente; uma fila FIFO (`push`/`shift`) distribuiria o desgaste de forma mais uniforme. O ponto mais crítico: não há evidência de um hook `onModuleDestroy` chamando `cleanIdle()` — sem ele, ao encerrar a aplicação, todas as conexões do pool serão abandonadas sem fechamento gracioso. Esse hook deveria ser adicionado explicitamente ao `ChatObjectPoolService` para garantir shutdown limpo. |

## Histórico de versões

| Versão | Data       | Descrição                                                                                                    | Autor                                               | Revisor                                             | Detalhamento da Revisão                                                                                                                                                                                             |
| :----- | :--------- | :----------------------------------------------------------------------------------------------------------- | :-------------------------------------------------- | :-------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `1.0`  | 21/05/2026 | Criação do documento com estrutura base, introdução, metodologia, estrutura de classes e rotas relacionadas. | [Ana Luiza](https://github.com/ana-pfeilsticker)    | [Mateus Magno](http://github.com/mtsmgn0)           | Revisando grafia, adicionando textos que compreendem melhor os assuntos abordados ecorrigindo a imagem do diagrama                                                                                                  |
| `1.1`  | 22/05/2026 | Revisão do padrão Object Pool e sua documentação.                                                            | [Miguel Arthur](https://github.com/zlimaz)          | [Vitor Hoffmann](https://github.com/vitor-hoffmann) | O uso de um 'loan pattern' (withConnection via try/finally) é excelente para evitar leaks. A lógica do pool, do limite máximo e a metodologia apresentada estão rigorosamente alinhadas às boas práticas do padrão. |
