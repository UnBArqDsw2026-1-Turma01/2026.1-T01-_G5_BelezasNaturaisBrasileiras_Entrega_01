...
classDiagram
    class ConfirmationCodeService {
        -instance: ConfirmationCodeService$
        -codigosEmitidos: Set~string~
        -CHARSET: string
        -CODIGO_LENGTH: number
        -constructor()
        +getInstance() ConfirmationCodeService$
        +gerarCodigo() string
        +validarCodigo(codigo: string) boolean
        +revogarCodigo(codigo: string) void
        +totalCodigosAtivos() number
    }
    ConfirmationCodeService --> ConfirmationCodeService : instância única
```
