...
classDiagram
  ...
  class UserBuilder {
    ...
    +withFotoPerfil(fotoPerfil string) UserBuilder
    +build() User
  }
  ...
  class CriarTrilhaUseCase {
    +execute(organizadorId string, input CriarTrilhaInput) Trilha
  }

  UserBuilder ..> User : build()
  TrilhaBuilder ..> Trilha : build()
  CriarTrilhaUseCase ..> TrilhaBuilder : usa
```
...
| `1.2`  | 20/05/2026 | Inclusão da aplicação adicional do Builder na criação de trilhas com `TrilhaBuilder`.                                           |                                                  |         |                         |
