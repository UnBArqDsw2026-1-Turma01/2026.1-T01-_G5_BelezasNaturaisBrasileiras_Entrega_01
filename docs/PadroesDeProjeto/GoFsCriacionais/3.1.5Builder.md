...
### Diagrama de Classes do Builder do User

![Diagrama de Classes do Builder do User](../../assets/fase_3/builder-diagrama.png)

### Diagrama de Classes do Builder de Trilhas

![Diagrama de Classes do Builder de Trilhas](../../assets/fase_3/builder-diagram.png)
...
| Integrante | Revisão |
| :--------- | :------ |
| [Vitor Hoffmann](https://github.com/vitor-hoffmann) | O trecho do `TrilhaBuilder` está truncado com `/* valida e instancia */`, não sendo possível verificar se os campos obrigatórios têm validação equivalente ao `UserBuilder`. A documentação lista `titulo`, `descricao`, `dataInicio` e `vagasMaximas` sem indicar quais são requeridos no `build()`. O `UserBuilder` usa `!this.id \|\| !this.email \|\| !this.nome`, que também rejeita strings vazias — comportamento esperado para campos obrigatórios. |
| [Ana Luiza Pfeilsticker](https://github.com/ana-pfeilsticker) | A API fluente com `withXxx().build()` é bem executada e a validação em `build()` antes da instanciação impede a criação de objetos inválidos. Um ponto de atenção: `UserBuilder` declara campos com non-null assertion (`private id!: string`), o que suprime checagens do compilador — declarar como `string | undefined` e centralizar a validação apenas no `build()` seria mais seguro em TypeScript. O trecho do `TrilhaBuilder` exibe `/* valida e instancia */` como placeholder; o `build()` completo deveria estar na documentação, pois é a parte mais crítica do padrão. Vale também documentar explicitamente a ausência de um `Director` como uma decisão consciente de variação em relação ao GoF clássico, distinguindo-a de uma omissão. |

## Histórico de versões

| Versão | Data       | Descrição                                                                                                                       | Autor                                               | Revisor                                             | Detalhamento da Revisão |
| :----- | :--------- | :------------------------------------------------------------------------------------------------------------------------------ | :-------------------------------------------------- | :-------------------------------------------------- | :---------------------- |
| `1.0`  | 18/05/2026 | Criação da estrutura do documento com seções de participantes, introdução, metodologia, estrutura de classes, diagrama e rotas. | [Ana Luiza](https://github.com/ana-pfeilsticker)    | [Vitor Hoffmann](https://github.com/vitor-hoffmann) | Verificação da correta aplicação do padrão Builder e clareza na documentação. |
