...
| :-------- | :----------------------------------------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 222015060 | [Ana Luiza](https://github.com/ana-pfeilsticker) | [949d533](https://github.com/UnBArqDsw2026-1-Turma01/2026.1-T01-_G5_BelezasNaturaisBrasileiras_Entrega_01/commit/949d53394883c7dfc90661547760bc5418f6e1d2) |
| 222021998 | [Mateus Magno](https://github.com/mtsmgn0)       | [949d533](https://github.com/UnBArqDsw2026-1-Turma01/2026.1-T01-_G5_BelezasNaturaisBrasileiras_Entrega_01/commit/949d53394883c7dfc90661547760bc5418f6e1d2) |
...
![Diagrama de Classes - Decorator](../../assets/fase_3/decorator.png)
...
### `CachedTrilhaRepository` — decorator de cache em memória

> [`backend/src/modules/trilhas/infrastructure/persistence/CachedTrilhaRepository.ts`](https://github.com/UnBArqDsw2026-1-Turma01/2026.1-T01-_G5_BelezasNaturaisBrasileiras_Entrega_01/blob/main/backend/src/modules/trilhas/infrastructure/persistence/CachedTrilhaRepository.ts)
...
export class CachedTrilhaRepository implements ITrilhaRepository {
  ...
  async findAll(): Promise<Trilha[]> {
    if (this.cacheAll !== null) return this.cacheAll;
  ...
  async save(trilha: Trilha): Promise<Trilha> {
    ...
    this.cacheAll = null; // invalida cache
    return saved;
  }
}
```

### `AuditedTrilhaRepository` — decorator de auditoria

> [`backend/src/modules/trilhas/infrastructure/persistence/AuditedTrilhaRepository.ts`](https://github.com/UnBArqDsw2026-1-Turma01/2026.1-T01-_G5_BelezasNaturaisBrasileiras_Entrega_01/blob/main/backend/src/modules/trilhas/infrastructure/persistence/AuditedTrilhaRepository.ts)
...
| Integrante                                          | Revisão                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| :-------------------------------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [Vitor Hoffmann](https://github.com/vitor-hoffmann) | A ordem `AuditedTrilhaRepository → CachedTrilhaRepository → PrismaTrilhaRepository` está correta: a auditoria antes do cache garante registro das operações reais, não das servidas pelo cache. O cache é in-memory por instância NestJS — em deployments horizontais com múltiplas instâncias, o cache ficaria desatualizado entre elas, o que exigiria um cache externo (ex.: Redis) para ser eficaz. O `AuditLog` também é in-memory e perde entradas em restart, o que limita sua utilidade para rastreabilidade em produção. |

## Histórico de versões

| Versão | Data       | Descrição                                                                              | Autor                                               | Revisor                                             | Detalhamento da Revisão                                                         |
| :----- | :--------- | :------------------------------------------------------------------------------------- | :-------------------------------------------------- | :-------------------------------------------------- | :------------------------------------------------------------------------------ |
| `1.0`  | 18/05/2026 | Criação do documento com estrutura base, introdução, quando aplicar e metodologia TDD. | [Ana Luiza](https://github.com/ana-pfeilsticker)    | [Vitor Hoffmann](https://github.com/vitor-hoffmann) | Verificação da correta aplicação do padrão Decorator e clareza na documentação. |
| `1.1`  | 18/05/2026 | Implementação completa: specs, decorators, AuditLog e wiring no módulo.                | [Ana Luiza](https://github.com/ana-pfeilsticker)    | [Vitor Hoffmann](https://github.com/vitor-hoffmann) | Verificação da implementação dos decorators de cache e auditoria.               |
