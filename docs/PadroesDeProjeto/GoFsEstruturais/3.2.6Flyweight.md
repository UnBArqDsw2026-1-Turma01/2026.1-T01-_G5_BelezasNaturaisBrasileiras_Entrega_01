...
classDiagram
    ...
    class LocalizacaoFlyweightFactory {
        -cache: Map~string, LocalizacaoFolha$
        +get(nome: string, descricao?: string)$ LocalizacaoFolha
        +totalInstancias$ number
        +limparCache()$ void
    }

    class LocalizacaoFolha {
        -nome: string
        -descricao: string
        +getNome() string
        +getTipo() string
    ...
    class ILocalizacaoComponent {
        <<interface>>
        +getNome() string
        +getTipo() string
        +getQuantidadePontos() number
        +adicionar(c: ILocalizacaoComponent)? void
        +remover(c: ILocalizacaoComponent)? void
        +listarFilhos()? ILocalizacaoComponent[]
    }

    class LocalizacaoComposita {
        -nome: string
        -tipo: string
        -filhos: ILocalizacaoComponent[]
    ...
    class TrilhasController {
        +contarPontosComComposite(body: LocalizacaoInput) object
    }

    ILocalizacaoComponent <|.. LocalizacaoFolha : implements
    ILocalizacaoComponent <|.. LocalizacaoComposita : implements
    LocalizacaoFlyweightFactory ..> LocalizacaoFolha : cria e cacheia
    LocalizacaoComposita o-- ILocalizacaoComponent : filhos[]
    TrilhasController ..> LocalizacaoFlyweightFactory : get(nome)
    TrilhasController ..> LocalizacaoComposita : constrói árvore

    note for LocalizacaoFlyweightFactory "Estado intrínseco:\nnome + descricao (imutável)\n\nEstado extrínseco:\nposição na árvore Composite\n(gerenciado por LocalizacaoComposita)"
```
...
| Integrante                                          | Revisão                                                                                                                                                                                                                                                                                                                                                                                                                           |
| :-------------------------------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [Vitor Hoffmann](https://github.com/vitor-hoffmann) | A chave `"${nome}::${descricao}"` pode gerar colisão se `nome` contiver `::` (ex.: `"Parque::Nacional"` com `descricao=""` colide com `"Parque"` e `descricao="::Nacional"`). O cache estático sem TTL é adequado para dados geográficos imutáveis, mas exige chamada explícita a `limparCache()` em testes para evitar interferência entre casos. O getter `totalInstancias` facilita monitoramento do uso do cache em produção. |

## Histórico de versões

| Versão | Data       | Descrição                                                                       | Autor                                               | Revisor                                             | Detalhamento da Revisão                                                                                             |
| :----- | :--------- | :------------------------------------------------------------------------------ | :-------------------------------------------------- | :-------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------ |
| `1.0`  | 21/05/2026 | Criação do documento com implementação completa do padrão Flyweight no projeto. | [Ana Luiza](https://github.com/ana-pfeilsticker)    | [Vitor Hoffmann](https://github.com/vitor-hoffmann) | Verificação da correta aplicação do padrão Flyweight e clareza na documentação.                                     |
| `1.1`  | 22/05/2026 | Revisão do padrão Flyweight                                                     | [Miguel Arthur](https://github.com/zlimaz)          | [Vitor Hoffmann](https://github.com/vitor-hoffmann) | Estrutura, diagramas UML e código condizem perfeitamente com o padrão Flyweight aplicado ao módulo de Localizações. |
