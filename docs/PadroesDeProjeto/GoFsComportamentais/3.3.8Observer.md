...
```mermaid
classDiagram
    class ITrilhaObserver {
        <<interface>>
        +onTrilhaFinalizada(trilhaId: string, participanteIds: string[]) Promise~void~
    }

    class TrilhaEventEmitter {
        -observers: ITrilhaObserver[]
        +subscribe(observer) void
        +unsubscribe(observer) void
        +notificarFinalizacao(trilhaId, participanteIds) Promise~void~
        +totalObservadores() number
    }

    class BadgeDistribuicaoObserver {
        -badgeRepository: IBadgeRepository
        +onTrilhaFinalizada(trilhaId, participanteIds) Promise~void~
    }
    ...
    class FinalizarTrilhaUseCase {
        +execute(trilhaId, organizadorId) Promise~void~
    }

    ITrilhaObserver <|.. BadgeDistribuicaoObserver
    ITrilhaObserver <|.. NotificacaoObserver
    TrilhaEventEmitter o-- ITrilhaObserver : observers
    FinalizarTrilhaUseCase --> TrilhaEventEmitter : notificarFinalizacao()
```
