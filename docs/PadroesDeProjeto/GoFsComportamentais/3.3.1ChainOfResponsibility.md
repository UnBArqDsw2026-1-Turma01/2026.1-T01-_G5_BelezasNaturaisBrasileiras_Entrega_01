...
O **Chain of Responsibility** é um padrão de projeto comportamental que permite passar solicitações por uma corrente de handlers. Ao receber uma solicitação, cada handler decide se processa a solicitação ou se a passa para o próximo handler na corrente.

Diferente de uma estrutura de decisão simples, o Chain of Responsibility permite que você transforme comportamentos específicos em objetos independentes chamados _handlers_. Isso permite que a aplicação decida dinamicamente qual objeto processará o pedido e em qual ordem, promovendo o princípio da responsabilidade única e o desacoplamento entre remetentes e destinatários.

## Quando Aplicar?

- Quando o programa deve processar diferentes tipos de solicitações de várias maneiras, mas os tipos exatos de solicitações e suas sequências são desconhecidos de antemão.
- Quando é essencial executar vários handlers em uma ordem específica.
- Quando o conjunto de handlers e sua ordem devem mudar durante o tempo de execução.
- Quando você deseja evitar o acoplamento excessivo entre o remetente de uma solicitação e seus receptores potenciais.

## Metodologia

O padrão Chain of Responsibility foi aplicado à **validação de criação de conta**. Antes de criar um usuário no Supabase Auth, o `CreateAccountUseCase` precisa garantir que o e-mail seja único, que a senha seja forte o suficiente e que os termos de uso foram aceitos. Sem o padrão, todas essas validações estariam acumuladas no próprio use case em uma sequência de blocos `if-else` complexos, tornando-o difícil de estender e testar.

Com o Chain of Responsibility, transformamos cada regra de validação em um handler independente. O use case monta a cadeia `EmailUniquenessHandler → PasswordStrengthHandler → TermsAcceptanceHandler` via `buildValidationChain()`. A solicitação percorre a corrente até que todos os handlers a processem com sucesso ou até que um deles lance uma exceção, interrompendo o processo. Isso garante que a lógica de negócio principal (interação com o Supabase) só seja executada se a solicitação passar por todos os filtros da corrente.

A classe abstrata `AccountValidationHandler` define o contrato de encadeamento: `setNext` configura o sucessor e `handle` gerencia o fluxo de execução. Cada handler concreto implementa apenas sua lógica específica no método `validate`. Esta abordagem facilita a adição de novas regras (como verificação de idade mínima ou blacklist de domínios) sem alterar o código existente, respeitando o Princípio Aberto/Fechado.
