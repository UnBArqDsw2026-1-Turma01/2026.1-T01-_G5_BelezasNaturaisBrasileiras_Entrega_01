...
classDiagram
    ...
    class BadgeDistribuicaoVisitor {
    ...
    class NotificacaoInscricaoVisitor {
    ...
    class Inscricao {
        ...
        +rejeitar() void
        +fazerCheckin() void
        +accept(visitor: IInscricaoVisitor) Promise~void~
    }
    ...
    class FinalizarTrilhaUseCase {
        ...
        -inscricaoRepository: IInscricaoRepository
        -visitors: IInscricaoVisitor[]
        +execute(trilhaId: string, organizadorId: string) Promise~void~
    }

    IInscricaoVisitor <|.. BadgeDistribuicaoVisitor : implements
    IInscricaoVisitor <|.. NotificacaoInscricaoVisitor : implements
    Inscricao --> InscricaoStatus : status
    Inscricao ..> IInscricaoVisitor : accept()
    FinalizarTrilhaUseCase --> IInscricaoVisitor : visitors[]
    FinalizarTrilhaUseCase ..> Inscricao : itera e chama accept()
```
...
| Integrante | Revisão |
| :--------- | :------ |
| [Vitor Hoffmann](https://github.com/vitor-hoffmann) | O duplo loop `visitors × inscricoes` é O(v × i); para trilhas com muitos participantes, agrupar inscrições por status antes de percorrer os visitors reduziria chamadas desnecessárias aos no-ops. Os no-ops explícitos (`visitAceita`, `visitRejeitada`, `visitPendente`) nos visitors concretos são corretos — garantem que o contrato da interface seja cumprido sem comportamento oculto. A injeção via token `IInscricaoVisitors` permite adicionar novos visitors apenas no módulo, sem alterar o use case. |

## Histórico de versões

| Versão | Data       | Descrição                                                                     | Autor                                            | Revisor | Detalhamento da Revisão |
| :----- | :--------- | :---------------------------------------------------------------------------- | :----------------------------------------------- | :------ | :---------------------- |
| `1.0`  | 21/05/2026 | Criação do documento com implementação completa do padrão Visitor no projeto. | [Ana Luiza](https://github.com/ana-pfeilsticker) | [Vitor Hoffmann](https://github.com/vitor-hoffmann) | Verificação da correta aplicação do padrão Visitor e clareza na documentação. |
| `1.1`  | 22/05/2026 | Revisão do padrão Visitor | [Miguel Arthur](https://github.com/zlimaz) | [Vitor Hoffmann](https://github.com/vitor-hoffmann) | Documento analisado. A estrutura do Visitor e o método accept() na entidade Inscricao foram implementados corretamente, resolvendo a segregação de responsabilidades no UseCase. |
