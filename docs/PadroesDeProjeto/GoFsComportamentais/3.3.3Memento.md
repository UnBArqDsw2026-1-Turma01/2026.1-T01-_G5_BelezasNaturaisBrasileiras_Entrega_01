...
classDiagram
  ...
  class RestaurarTrilhaUseCase {
    -trilhaRepository ITrilhaRepository
    -caretaker TrilhaCaretaker
    +execute(trilhaId string, organizadorId string) Promise~Trilha~
  }

  TrilhaMemento --> TrilhaMementoState : armazena
  TrilhaCaretaker o-- TrilhaMemento : guarda histórico
  Trilha ..> TrilhaMemento : cria via saveState
  FinalizarTrilhaUseCase --> TrilhaCaretaker : salva snapshot
  FinalizarTrilhaUseCase --> Trilha : finaliza
  RestaurarTrilhaUseCase --> TrilhaCaretaker : restaura snapshot
  RestaurarTrilhaUseCase --> Trilha : aplica estado
```
