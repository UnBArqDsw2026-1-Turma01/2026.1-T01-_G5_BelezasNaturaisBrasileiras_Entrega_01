...
* **Rastreabilidade com a Fase de Concepção (5W2H):** A elaboração da Modelagem Dinâmica possui rastreabilidade direta e rigorosa com as definições estratégicas consolidadas no **5W2H** definido durante a etapa de Design Sprint. As diretrizes de negócio estabelecidas nesse framework (respostas ao Quem, O quê, Por que e Como) serviram como insumo primordial para determinar o comportamento temporal, as restrições de acesso e as transações de estado da plataforma. Primeiramente, a definição exata de Quem (Who) atua no sistema estabeleceu a hierarquia de privilégios (Visitante, Comum, Organizador e Administrador). Essa premissa de negócio estruturou o Diagrama de Estados, que mapeia e isola as rotas possíveis dependendo do ator ativo. Essa mesma hierarquia fundamentou o Diagrama de Sequência de Upgrade de Conta, exigindo a aprovação explícita do Administrador para a instanciação de um novo perfil. A resposta ao Por quê (Why) — que prioriza a segurança dos usuários e a moderação constante — norteou o fluxo de Moderação e Governança, traduzindo a política de proteção em uma barreira arquitetural na qual apenas a classe Administrador possui capacidade técnica para invocar métodos de exclusão definitiva. No que tange ao uso prático do sistema, as respostas ao 'O que?' (What) e 'Como?' (How) moldaram os fluxos operacionais da aplicação. O fluxo de Criação e Edição de Ponto Turístico atende à demanda de uma base colaborativa construída pela comunidade, garantindo a rastreabilidade dos metadados geográficos através do encapsulamento da Regra de Negócio (RN01), onde o próprio domínio registra a autoria e a data das modificações. Por fim, o Diagrama de Sequência de Gestão e Inscrição em Trilhas materializa o motor de eventos desenhado na Fase 1. Ele mitiga os riscos inerentes à organização no ecoturismo estruturando as interações de forma síncrona e bloqueante: o sistema exige a aprovação de convites, gerencia um chat privado 1-para-1 (evitando exposição pública) e impõe a geração de Códigos de Confirmação. O fluxo garante que a gamificação (distribuição de Badges) e o direito de avaliação final ocorram estritamente após a "Validação de Presença Física" gerida pelo Organizador.

---

### 2.2.1. Diagrama de Estados

O diagrama de estados modela o ciclo de vida e as transições de permissões do usuário dentro da plataforma **Belezas Naturais Brasileiras**. Ele utiliza **Estados Compostos** para segregar as funcionalidades de acordo com o nível de autenticação e o papel (*role*) assumido.

Este artefato é a materialização técnica do **RBAC (Role-Based Access Control)** definido no glossário. A transição entre os estados reflete o rigor das **Regras de Negócio (RN)** mapeadas nos diagramas **BPMN**, garantindo que a elevação de privilégios (de Usuário Comum para Organizador) passe por uma esteira de validação administrativa, mitigando riscos de fraudes em expedições.

#### Versão 3 (Final — Completude de notações e Elos) 

![Diagrama de Estados VF](../assets/fase_2/images/Diagrama_de_Estados_vf.png)

**Mudanças e Melhorias Implementadas na V3 (Versão Final):**

* **Completude de Eventos:** Adição de descrições em todas as transições (setas), incluindo fluxos de saída (*Logout*) e inicialização de sub-estados, garantindo rastreabilidade total do comportamento da **SPA**.
* **Sincronização Estratégica (5W2H):** Inclusão elementos que demonstram as travas de estado (como o Chat 1-para-1) para mitigar fraudes e o uso de dispositivos móveis para o **Check-in Físico**.
* **Refinamento do Ciclo de Gamificação:** A transição para "Finalizar Trilha" agora aciona explicitamente o gatilho de distribuição automática de **Badges (Insígnias)** e liberação de comentários, conforme a **RN05**.
* **Governança e RBAC:** Detalhamento rigoroso das permissões do Administrador (RF11), separando a gestão de privilégios da moderação de conteúdo, espelhando o processo de "Sanar Impedimentos" do **BPMN**.
* **Padronização Semântica:** Alinhamento total com o **Glossário**, utilizando termos como "Código de Confirmação" e "Check-in Físico" nas transições para eliminar a **Carga Cognitiva** durante a leitura do fluxo.

#### Referência Histórica (V2 - Refinamento de Legibilidade)
![Diagrama de Estados V2](../assets/fase_2/images/Diagrama_de_Estados-v2.png)
* **Mudança principal:** Substituição de IDs técnicos (RF01, etc.) por descrições funcionais para reduzir a **Carga Cognitiva**, conforme sugestão do grupo na reunnião de feedbacks.

#### Referência Histórica (V1 - Base Inicial)
![Diagrama de Estados V1](../assets/fase_2/images/Diagrama_de_Estados.png)
...
O diagrama modela o fluxo de **criação e edição de pontos turísticos** (RF03, RN01), cobrindo dois cenários alternativos: o cadastro de um novo ponto e a atualização de um ponto já existente. Em ambos os casos, o ator principal é o **Usuário Autenticado**, que interage com o objeto `PontoTuristico` e a camada de persistência.

#### Análise da Versão 2 (Atualizada)

![Diagrama de Sequência — Criação e Edição de Ponto Turístico](../assets/fase_2/images/PontosT_v2.png)

**Mudanças e Melhorias Implementadas na V2:**
* **Correção do Fluxo Principal (RF03):** Retificação do erro descritivo da versão anterior, que tratava de "Inscrição em Trilhas". O fluxo agora detalha corretamente a criação e edição de pontos turísticos (Título, Descrição e Localização).
* **Visibilidade da Persistência (RN01):** Inclusão do participante **Banco de Dados** para demonstrar a gravação física do registro e, principalmente, do log de versionamento.
* **Detalhamento do Versionamento:** Explicitação das mensagens de `INSERT` na tabela de logs durante o cenário de edição, garantindo que o `id_autor` e a `data_edicao` sejam registrados conforme a **RN01**.
* **Respostas de UI:** Refinamento das mensagens de confirmação retornadas ao usuário, indicando o sucesso da operação e o registro da nova versão.

**Fluxo Principal Corrigido (V2):**
1. Usuário Autenticado → PontoTurístico: `criarLocal(dados)` ou `editarDados(novosDados)`
2. PontoTurístico → Banco de Dados: `INSERT/UPDATE (dados do local)`
3. PontoTurístico → Banco de Dados: `INSERT (log de versionamento - RN01)`
4. Banco de Dados -->> PontoTurístico: `confirmação_persistência`
5. PontoTurístico -->> Usuário Autenticado: `UI: "Operação realizada com sucesso"`

#### Referência Histórica Pré-Feedback do Grupo (Versão 1)

![Diagrama de Sequência — Criação e Edição de Ponto Turístico](../assets/fase_2/images/PontosT.png)

*Nota: A versão anterior descrevia erroneamente o fluxo de inscrição em trilhas sob o título de criação de pontos turísticos.*
...
## Participação da criação dos modelos iniciais
...
> SERRANO, Milene. **Material do Aprender 3 da UnB - Módulo Notação UML - Modelagem Dinâmica**. [S.l.]: Milene Serrano, 2026.
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| `1.0` | 21/04/2026 | Criação do documento de Modelagem Dinâmica. | [Antonio Carvalho](https://github.com/antonioscarvalho) | [Mateus Magno](https://github.com/mtsmgn0) | **Revisão aprovada:** Estrutura inicial validada com introdução da modelagem dinâmica e definição dos objetivos. |
| `2.0` | 21/04/2026 | Inserção do diagrama de sequência do fluxo de vida da trilha. | [Mateus Magno](https://github.com/mtsmgn0) | [Mário Vinícius](https://github.com/MarioViniciusBC) | **Revisão aprovada:** Fluxo validado cobrindo o ciclo completo da trilha (criação à finalização). |
| `3.0` | 21/04/2026 | Inserção do diagrama de Upgrade de Conta. | [Mário Vinícius](https://github.com/MarioViniciusBC) | [Mateus Magno](https://github.com/mtsmgn0) | **Revisão aprovada:** Diagrama validado quanto ao fluxo de solicitação e análise de upgrade (RBAC). |
| `3.1` | 21/04/2026 | Ilustração do caminho feliz (Upgrade de Conta). | [Mário Vinícius](https://github.com/MarioViniciusBC) | [Antonio Carvalho](https://github.com/antonioscarvalho) | **Revisão aprovada:** Caminho feliz validado, destacando a transição de permissões. |
| `3.2` | 21/04/2026 | Inserção do diagrama de Moderação e Governança. | [Mário Vinícius](https://github.com/MarioViniciusBC) | [Antonio Carvalho](https://github.com/antonioscarvalho) | **Revisão aprovada:** Fluxo de moderação validado, separando denúncia, análise e decisão (RN02). |
| `4.0` | 23/04/2026 | Inserção da tabela de feedbacks para os diagramas. | [Mateus Magno](https://github.com/mtsmgn0) | [Mário Vinícius](https://github.com/MarioViniciusBC) | **Revisão aprovada:** Inserção da tabela de feedbacks com pontos de revisão validados. |
| `4.1` | 23/04/2026 | Atualização e padronização do histórico de versões. | [Mateus Magno](https://github.com/mtsmgn0) | [Mário Vinícius](https://github.com/MarioViniciusBC) | **Revisão aprovada:** Padronização conforme feedback da equipe validada. |
| `5.0` | 23/04/2026 | Refatoração do Diagrama de Estados e Sequência (RF03). | [Antonio Carvalho](https://github.com/antonioscarvalho) | [Mateus Magno](https://github.com/mtsmgn0) | **Revisão aprovada:** Correção crítica no contexto do RF03 (Ponto Turístico), inclusão da persistência de versionamento (RN01) e legibilidade nas transições de estados. |
| `5.1` | 23/04/2026 | Integração estratégica com o planejamento 5W2H. | [Antonio Carvalho](https://github.com/antonioscarvalho) | [Mário Vinícius](https://github.com/MarioViniciusBC) | **Revisão aprovada:** Inserção de pequenos elementos nos diagramas correlacionando os fluxos técnicos aos objetivos de segurança, mobilidade e cronograma do 5W2H (ex.: comunicação direta (chat 1-para-1). |
| `5.2` | 24/04/2026 |Inclusão do elo de rastreabilidade com a Fase 1 (5W2H) na Metodologia. | [Mário Vinícius](https://github.com/MarioViniciusBC) | [Mateus Magno](https://github.com/mtsmgn0)   | **Revisão aprovada:** O elo de rastreabilidade realmente reflete a maneira que a aplicação funciona, e destaca elementos realmente usados para a produção dos diagramas nessa etapa. |
| `6.0` | 24/04/2026 | Consolidação teórica e técnica da Modelagem Dinâmica Versão Final do Diagrama de Estados. | [Antonio Carvalho](https://github.com/antonioscarvalho) | [Mário Vinícius](https://github.com/MarioViniciusBC) | **Revisão aprovada:** Integração profunda com os artefatos de Glossário e BPMN. Validada a inclusão de conceitos como RBAC, Check-in Físico e Histórico de Edição (RN01) nos diagramas de Estados e Sequência. |
