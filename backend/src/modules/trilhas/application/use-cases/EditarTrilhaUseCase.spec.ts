...
const makeTrilha = (): Trilha =>
  new Trilha('t1', 'Original', 'Desc', 'org-1', 'Ponto A', new Date('2026-06-01'), 20, TrilhaStatus.ATIVA);

describe('EditarTrilhaUseCase', () => {
  const mockRepo = { findById: jest.fn(), save: jest.fn(), create: jest.fn(), findAll: jest.fn() };
  let useCase: EditarTrilhaUseCase;

  beforeEach(() => {
    jest.clearAllMocks();
    useCase = new EditarTrilhaUseCase(mockRepo as any);
  });
  ...
  it('should edit trilha and save it', async () => {
    ...
    const result = await useCase.execute('t1', 'org-1', { titulo: 'Novo Título' });
  ...
  it('should throw NotFoundException when trilha not found', async () => {
    ...
    await expect(useCase.execute('t1', 'org-1', { titulo: 'X' })).rejects.toThrow(NotFoundException);
  });

  it('should throw ForbiddenException when caller is not the organizer', async () => {
    mockRepo.findById.mockResolvedValue(makeTrilha());
    await expect(useCase.execute('t1', 'outro-org', { titulo: 'X' })).rejects.toThrow(ForbiddenException);
  });
