...
describe('Observer — TrilhaEventEmitter', () => {
  ...
  describe('subscribe()', () => {
    it('deve registrar um observador', () => {
      const observer = new BadgeDistribuicaoObserver();
      emitter.subscribe(observer);

      expect(emitter.totalObservadores).toBe(1);
    });

    it('não deve registrar o mesmo observador duas vezes', () => {
      const observer = new BadgeDistribuicaoObserver();
      emitter.subscribe(observer);
      emitter.subscribe(observer);
  ...
  describe('notificarFinalizacao()', () => {
    it('deve chamar onTrilhaFinalizada em todos os observadores', () => {
      const fn1 = jest.fn();
      const fn2 = jest.fn();
      ...
      emitter.notificarFinalizacao('trilha-123', ['user-1', 'user-2']);

      expect(fn1).toHaveBeenCalledWith('trilha-123', ['user-1', 'user-2']);
      expect(fn2).toHaveBeenCalledWith('trilha-123', ['user-1', 'user-2']);
    });

    it('não deve chamar observadores removidos', () => {
      const fn = jest.fn();
      const observer: ITrilhaObserver = { onTrilhaFinalizada: fn };
      emitter.subscribe(observer);
      emitter.unsubscribe(observer);

      emitter.notificarFinalizacao('trilha-456', ['user-3']);

      expect(fn).not.toHaveBeenCalled();
    });

    it('deve notificar com lista vazia quando nenhum participante é válido', () => {
      const fn = jest.fn();
      const observer: ITrilhaObserver = { onTrilhaFinalizada: fn };
      emitter.subscribe(observer);

      emitter.notificarFinalizacao('trilha-789', []);
  ...
  describe('NotificacaoObserver', () => {
    it('deve instanciar sem erros', () => {
      const observer = new NotificacaoObserver();
      expect(() =>
        observer.onTrilhaFinalizada('trilha-1', ['user-a']),
      ).not.toThrow();
    });
  });
});
